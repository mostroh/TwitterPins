...
/*
Credits to https://gist.github.com/polson/227e1a039a09f2728163bf7235990178
 */
...
class Oauth1SigningInterceptor(val oauthKeys: OauthKeys,
    ...
    fun signRequest(request: Request): Request {
        ...
        val parameters = hashMapOf(
            ...
            OAUTH_NONCE to re.replace(nonce,""),
        ...
        request.body()?.let {
            it.asString().split('&')
                ...
                ?.filter { filtered ->
                    (filtered.size == 2).also { hasTwoParts ->
                        if (!hasTwoParts) throw IllegalStateException("Key with no value: ${filtered.getOrNull(0)}")
                    }
                }
                ?.associate {
                    val (key, value) = it
